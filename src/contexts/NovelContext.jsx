...
export function NovelProvider({ children }) {
    ...
    const handleMergeConflict = useCallback(async (selections) => {
        ...
        try {
            ...
            // 5. 构造最终数据包
            // 注意：分类等元数据通常以服务器为准，暂不混合
            // [修复] 不再前端自增版本号，版本号由后端统一控制
            // 使用服务器当前版本，AutoSave 会通过内容对比检测到变化并同步
            // 后端写入成功后会自动递增版本号并返回
            ...
            const finalState = {
                ...
                charCats: snapshot.charCats || charCats,
                sceneCats: snapshot.sceneCats || sceneCats,
                settingCats: snapshot.settingCats || settingCats,
